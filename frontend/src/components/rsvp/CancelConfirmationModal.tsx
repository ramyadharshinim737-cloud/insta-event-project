...
const CancelConfirmationModal: React.FC<Props> = ({ visible, onConfirm, onClose }) => {
  const { colors } = useTheme();
  return (
    <Modal visible={visible} transparent animationType="fade">
      <View style={styles.backdrop}>
        <View style={[styles.container, { backgroundColor: colors.background, borderColor: colors.border }]}>
          <Text style={[styles.title, { color: colors.text }]}>Cancel RSVP?</Text>
          <Text style={{ color: colors.text, marginBottom: 16 }}>This will free your spot. You can RSVP again if seats are available.</Text>
          <View style={{ flexDirection: 'row', gap: 8 }}>
            <TouchableOpacity onPress={onClose} style={[styles.button, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <Text style={{ color: colors.text }}>Keep RSVP</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={onConfirm} style={[styles.button, { backgroundColor: colors.primary, borderColor: colors.border }]}>
              <Text style={{ color: '#fff' }}>Cancel RSVP</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
